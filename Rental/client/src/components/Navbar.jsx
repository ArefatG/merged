...
const Navbar = () => {
  ...
  const handlePortfolioClick = () => {
    if (userData && userData.length > 0) {
      const userInfo = encodeURIComponent(JSON.stringify(userData[0]));
      window.location.href = `http://localhost:3001/login?user=${userInfo}`;
    } else {
  ...
  const navItems = (
    <>
      ...
      <li>
        <a onClick={handlePortfolioClick} style={{ cursor: "pointer" }}>
          Portfolio
        </a>
      </li>
    </>
            ...
            >
              ...
              <li className="mt-3">
                <form className="w-full" onSubmit={handleSearchSubmit}>
                  <input
                    type="text"
                    className="input input-bordered w-full py-1 px-2"
